...
export interface ParsedRouteData {
    ...
    routeType: 'Service' | 'Local Service' | 'Outstation Route' | 'Outstation Route Fare' | 'Driver Service' | 'Unknown';
    isLegacyPhp: boolean;
    slugs: string[];
}
...
export function parseUrlSlug(slugs: string[]): ParsedRouteData {
    ...
    if (lowerSegment.includes('tempo') || lowerSegment.includes('traveller')) {
        ...
        vehicle = 'Car';
    } else if (lowerSegment.includes('driver') || lowerSegment.includes('chauffeur')) {
        vehicle = 'Driver';
    ...
    if (lowerSegment.includes('driver') || lowerSegment.includes('chauffeur')) {
        ...
        if (inMatch) {
            ...
            if (!['driver', 'chauffeur', 'hire', 'service', 'rent'].includes(possibleCity)) {
                origin = possibleCity;
            }
        }
    }
    // 3. Outstation Route parsing (-to-)
        ...
        routeType = lowerSegment.endsWith('-fare') ? 'Outstation Route Fare' : 'Outstation Route';
    ...
    // 3. Local City Service Parsing
        ...
        (lowerSegment.includes('-') && !['rent', 'taxi', 'cab', 'cabs', 'hire', 'airport', 'area', 'condition', 'trip', 'driver', 'agencies', 'company', 'agency', 'operators'].includes(lowerSegment.split('-').pop() || ''))
    ) {
        ...
        if (inMatch) {
            ...
            origin = fromMatch[1];
            if (origin === 'airport' || origin === 'local') origin = slugs.length > 1 ? slugs[0] : null;
    ...
    // Capitalize Helper
    const capitalize = (str: string | null) => str ? str.charAt(0).toUpperCase() + str.slice(1) : str;
    ...
    } else if (lowerVehicle.includes('driver') || lowerVehicle.includes('chauffeur')) {
