...
export default function LocalDominance({ city }: LocalDominanceProps) {
  const data =
    localDominanceData[city.toLowerCase() as keyof typeof localDominanceData];
            ...
            >
              ...
              <a
                href={`https://wa.me/916280820037?text=${encodeURIComponent(
                  `Hi Chiku Cabs, I’m interested in booking a cabs in ${data.city} through your website. Please share the available options and booking details.`,
                )}`}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-8 inline-flex w-full rounded-xl bg-primary px-3 py-3 font-semibold text-white hover:opacity-90"
              >
